...
-(UIImage *)imageByScalingToSize:(CGSize)size andCroppingWithRect:(CGRect)rect {
    ...
    CGContextScaleCTM(ctx1, scaleFactor, scaleFactor);
    
    CGContextConcatCTM(ctx1, transform);
    switch (self.imageOrientation) {
        case UIImageOrientationLeft:
        case UIImageOrientationLeftMirrored:
        case UIImageOrientationRight:
        case UIImageOrientationRightMirrored:
            CGContextDrawImage(ctx1, CGRectMake(0,0, self.size.height, self.size.width), self.CGImage);
            break;
            
        default:
            CGContextDrawImage(ctx1, CGRectMake(0,0, self.size.width, self.size.height), self.CGImage);
            break;
    }
    
    // Create a new UIImage from the drawing context
    CGImageRef cgimg = CGBitmapContextCreateImage(ctx1);
    UIImage *img = [UIImage imageWithCGImage:cgimg scale:scaleFactor orientation:UIImageOrientationUp];
    ...
    // Draw into the context; this scales the image
    CGContextDrawImage(ctx, newRect, imageRef1);
    
    // Get the resized image from the context and a UIImage
    CGImageRef newImageRef1 = CGBitmapContextCreateImage(ctx);
    UIImage *newImage = [UIImage imageWithCGImage:newImageRef1 scale:self.scale orientation:UIImageOrientationUp];
